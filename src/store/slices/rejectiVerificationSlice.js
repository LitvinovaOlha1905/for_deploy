...
export const rejectVerificationStatus = createAsyncThunk(
	...
	async ({ documentVerificationId, rejectionMessage, bearerToken }) => {
		...
		const url = `${apiUrl}/bonus-program/updateDocumentVerificationStatus?documentVerificationId=${documentVerificationId}&status=REJECTED&rejectionMessage=${encodeURIComponent(
			rejectionMessage
		)}`;
